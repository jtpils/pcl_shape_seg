...
def evaluate():
    ...
    for room_path in ROOM_PATH_LIST:
        ...
        print(f'room_path, out_data_label_filename = {room_path}, {out_data_label_filename}')
        a, b = eval_one_epoch(sess, ops, room_path, out_data_label_filename, out_gt_label_filename)
        total_correct += a
        total_seen += b
        fout_out_filelist.write(out_data_label_filename+'\n')
    fout_out_filelist.close()
    log_string('all room eval accuracy: %f'% (total_correct / max(float(total_seen), 1)))
...
def eval_one_epoch(sess, ops, room_path, out_data_label_filename, out_gt_label_filename):
    ...
    for batch_idx in range(num_batches):
        ...
        for b in range(BATCH_SIZE):
            ...
            pts[:,3:6] *= 255.0
            pred = pred_label[b, :]
#           if 'chair' in out_data_label_filename:
#           print(f'pred_label[b, :] = {pred_label[b, :]}')
#           print(f'min(pred), max(pred) = {min(pred)}, {max(pred)}')
