...
DATA_PATH = os.path.join(ROOT_DIR, 'data', 'sem_seg_shapes')
...
#g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'shapes/class_names.txt'))]
g_classes = ['cone', 'cylinder', 'sphere', 'torus']
g_class2label = {cls: i for i,cls in enumerate(g_classes)}
g_class2color = {'cone': GREEN,
                 'cylinder': BLUE,
                 'sphere': RED,
                 'torus': ORANGE}
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    This separates the data into a grid.
    """
...
#           continue
        ...
        # Print out slices here
#       print(f'block_data.shape = {block_data.shape}')
#       print(f'block_label.shape = {block_label.shape}')
#       colors = [g_label2color[label] for label in block_label]
#
#       points = np.asarray(block_data[:,0:3], dtype=np.float32)
